...
class MyoRaw(object):
    ...
    def __init__(self, tty=None):
        ...
        self.set_leds([255, 255, 255], [128, 128, 255])
        self.set_mode(EmgMode.OFF, ImuMode.OFF, ClassifierMode.ON)
        self.sleep_mode(1)
        # print("the battay:" , self.battery())
    ...
    def battery(self):
        hex_batt = self.read_attr(ReadHandle.BATTERY)
        batt = ord(hex_batt)
        return batt

    '''write + add'''
    def set_leds(self, logo, line):
        	self.write_attr(WriteHandle.COMMAND, struct.pack('<8B', 6, 6, *(logo + line)))
    def power_off(self):
        self.write_attr(WriteHandle.COMMAND, b'\x04\x00')
    def sleep_mode(self, mode):
        self.write_attr(WriteHandle.COMMAND, struct.pack('<3B', 9, 1, mode)) 
    def vibrate(self, length):
        if length in range(1, 4):
            self.write_attr(WriteHandle.COMMAND, struct.pack('<3B', 3, 1, length))  
    # Suscribe to battery notifications
    def battery_notifications(self):
        self.write_attr(WriteHandle.BATTERY, b'\x01\x10')
    # Suscribe to raw EMG notifications
    def emg_raw_notifications(self):
        self.write_attr(WriteHandle.EMG0, b'\x01\x00')
        self.write_attr(WriteHandle.EMG1, b'\x01\x00')
        self.write_attr(WriteHandle.EMG2, b'\x01\x00')
        self.write_attr(WriteHandle.EMG3, b'\x01\x00')
    # Suscribe to filtered EMG notifications
    def emg_filt_notifications(self):
        self.write_attr(WriteHandle.EMG_FILT, b'\x01\x00')
    # Suscribe to IMU notifications
    def imu_notifications(self):
        self.write_attr(WriteHandle.IMU, b'\x01\x00')
    # Suscribe to classifier notifications
    def classifier_notifications(self):
        self.write_attr(WriteHandle.CLASSIFIER, b'\x02\x00')
    def set_mode(self, emg_mode, imu_mode, classifier_mode):
        command_string = struct.pack('<5B', 1, 3, emg_mode, imu_mode, classifier_mode)
        self.write_attr(WriteHandle.COMMAND, command_string)
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            if attr == 0x23:
            	data_type, value, address, _, _, _ = unpack('6B', pay)
            	if data_type == 3:
            		self.on_pose( Pose(value))
            elif attr == 0x27:
                vals = unpack('8HB', pay)
                emg = vals[:8]
                moving = vals[8]
